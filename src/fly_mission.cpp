        ...
        switch(yolo_detect_mode){
          ...
          case follow_mode:
            ...
            if(land_color.compare(takeoff_color) == 0 && hypot(yolo_result.point.x, yolo_result.point.y) < 0.1){
              ...
              end_y = local_pos.pose.pose.position.y;
              last_request = ros::Time::now();
              break;
            }
            ...
            for(int i = 0; i < 2; i++){
              yolo_vel[i] = satfunc(yolo_vel[i], err_max);
            }
            ROS_WARN("Velocity Command Body before CA: vx: %.2f , vy: %.2f ", yolo_vel[0], yolo_vel[1]);
            setpoint_raw.type_mask = 1 + 2 /*+ 4  +8 + 16 */+ 32 + 64 + 128 + 256 + 512 /*+ 1024 */ + 2048;
            ...
            break;
          // case holding_mode:
          //   ROS_INFO("holding mode");

          //   if(mission_pos_cruise(end_x, end_y, ALTITUDE, 0, 0.01)){
          //     mission_num = 4;
          //     break;
          //   }
          //   setpoint_raw.type_mask = 1 + 2 /*+ 4  +8 + 16 */+ 32 + 64 + 128 + 256 + 512 /*+ 1024 */ + 2048;
